	...
	calendar.events.list({
		...
		if (events.length == 0) {
			...
			for (var i = 0; i < events.length; i++) {
				...
				//if(end.diff(start) > 1800000) {
					//winston.debug('Duration exceeds 30 minutes. Adding events every minute until end time.’);
 ...
 * If it is the first time run, it will do the callback immediately.
